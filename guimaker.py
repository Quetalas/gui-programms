"""
Frame, автоматически создающий меню и панели инструментов в окне.
"""
import sys
from tkinter import *
...
class GuiMaker(Frame):
    ...
    def makeWidgets(self):
        name = Label(self, height=10, width=40,
    ...
    def start(self):
        """Переопределитев подклассе
        """
        pass

    def makeMenuBar(self):
        """
        Создаёт полосу меню вверху
        :return:
        """
        menubar = Frame(self, relief=RAISED, bd=0)
    ...
    def makeToolBar(self):
        if self.toolBar:
            toolBar=Frame(self, cursor='hand2', relief=SUNKEN, bd=0)
            toolBar.pack(side=BOTTOM, fill=X)
            for name, action, where in self.toolBar:
                Button(toolBar, text=name, command=action, bd=0).pack(where)
